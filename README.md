...
本仓库是基于 AiCube 创建的 AI8051U Keil C251 工程，用于精密电流源数字控制端。当前数字系统负责设定电流输入、数码管显示、OLED 波形显示、UART PID 调参、DAC 输出控制框架，以及片内/外挂 ADC 采样框架。
...
- 当前可用功能：
  ...
  - 片内 ADC0 采样运放输出波形，SSD1315 I2C OLED 显示 128x64 实时波形。
  - ADS1110A0 外挂 ADC 已作为实际电流和 PID 数据源接入代码，默认 16bit / 15SPS / PGA=1。
  - UART1 通过 CH340 做状态回传和 PID 参数调节。
  - PID 框架与 IAP 参数保存框架已加入。
- 待硬件验证：
  - DAC8311IDCKR 外挂 DAC 输出。
  - ADS1110A0IDBVR 外挂精密 ADC 实物采样。
  - PID 对真实模拟闭环的调节效果。
...
| 片内 ADC | ADC0 | AiCube 配置 | OLED 波形输入 |
| 片内 ADC | ADC15 | 内部通道 | 用于估算 VCC/Vref，服务 OLED 标尺和串口状态 |
| ADS1110A0 | SCL / SDA | P1.5 / P1.4 | 与 OLED 共用 I2C，作为实际电流/PID 数据源，待实物验证 |
...
- SUB 长按切换 OLED 示波器模式：全局模式显示 ADC 全量程波形，纵向放大模式根据最近 128 点均值和平均绝对偏差自动缩放显示范围。

### ADC 与实际电流

代码位置：`Sources/adc.c`、`Sources/inc/adc.h`、`Sources/i2c.c`、`Sources/inc/i2c.h`

- 当前默认 `CURRENT_SAMPLE_SOURCE = CURRENT_SAMPLE_SOURCE_ADS1110`。
- 实际电流和 PID 数据源来自 ADS1110A0，OLED 波形显示仍使用片内 ADC0。
- ADS1110 配置为连续转换、15SPS、16bit、PGA=1；读取结果为 16 位有符号二补码，当前电路只使用正半量程，负值在电流换算时钳为 0。
- 采样电阻 `CURRENT_SENSE_RES_MOHM = 100mOhm`。
- OPA189 检流放大倍数按定点数表示为 `CURRENT_AMP_GAIN_NUM = 101`、`CURRENT_AMP_GAIN_DEN = 10`，即 10.1 倍。
...
- 片内 ADC15 仍会在空闲时估算 VCC/Vref，用于 OLED 标尺和串口状态，不作为精密实际电流/PID 数据源。
- 如果希望更接近 ADS1110 的 15SPS 满速，可在 AiCube 中把 Timer3 周期从 100ms 调整到 50~67ms；无需新增定时器。
...
- OLED 示波器支持全局/纵向放大两种模式。纵向放大模式保存 128 个原始 ADC 采样值，刷新时实时计算窗口均值和平均绝对偏差，并设置最小缩放窗口，适合观察稳定直流附近的小纹波。
...
- PID 控制任务当前在 ADS1110 实际电流刷新后由主循环调用，最终 DAC 输出为“前馈查表码 + PID 反馈修正量”。
...
- 默认表按理想状态初始化：`100mOhm` 采样电阻、OPA189 放大 `10.1` 倍、`DAC8311_REF_MV = 5000mV`。
...
- 当前已作为实际电流和 PID 数据源接入代码，但硬件尚未到货，I2C 通信、输入极性、噪声和量程仍待验证。
...
- 已完成：AiCube 工程配置、数码管扫描、按键调节、防抖、长按模式切换、SSD1315 I2C OLED 波形显示、ADS1110 实际电流/PID 数据源代码接入、UART 调参、PID/IAP 框架。
